...
bool inLHSOfBinaryAssignOperator(ASTContext &Ctx, Expr *E) {
  while (BinaryOperator *op = getParentNodeOfType<BinaryOperator>(Ctx, E)) {
...
public:
  ...
  bool visitArrayTypedConstructorExpr(const CXXConstructExpr *expr) {
    ...
    auto *elementDecl = elementType->getAsRecordDecl();
...
public:
  ...
  bool VisitMemberExpr(MemberExpr *expr) {
    ...
    } else if (parent.get<CompoundAssignOperator>()) {
...
public:
  ...
  bool VisitArraySubscriptExpr(ArraySubscriptExpr *e) {
    if (getParentNodeOfType<ArraySubscriptExpr>(Ctx, e)) return true; // we're in the middle of a multi-dim arr ref
    if (getParentNodeOfType<ArrayInitLoopExpr>(Ctx, e)) return true; // we're in the middle of an autogenerated constructor
...
public:
  ...
  bool VisitArraySubscriptExpr(ArraySubscriptExpr *e) {
    ...
    auto *binaryExpr = getParentNodeOfType<BinaryOperator>(Ctx, e);
      ...
      if (llvm::isa<CXXRecordDecl>(D)) {
          ...
          for (auto *method : decl->methods()) {
              ...
              if (llvm::isa<CXXConstructorDecl>(method)) {
                  ...
                  auto methodStr = getMethod(constr, compressionCodeGen.getFullyQualifiedCompressedStructName(), compressionCodeGen.getCompressedStructName()) + "\n\n";

                  // when constructor definitions are provided in headers, they are immune to ODR
                  // thus, we have to mark them as inline to avoid ODR too
                  // but somehow this in only an issue with constructors that have initialisers (call super constructors)
                  methods += (constr->getNumCtorInitializers() > 0 ? "inline " : "") + methodStr;
              } else {
