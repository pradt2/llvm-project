...
const char *TEMPLATES = "#include <algorithm>\n"
                        ...
                        "template<typename T, int BITS, int ALIGNMENT, bool LSB> requires (BITS <= 64 && ALIGNMENT <= 8)\n"
                        ...
                        "\n"
                        "        num = this->data;\n"
                        "        if constexpr (!LSB) num <<= sizeof(T) * 8 - BITS;\n"
                        "\n"
                        "        return orig;\n"
                        "    }\n"
                        "\n"
                        ...
                        "using Native = std::conditional_t<std::is_floating_point_v<T>, std::conditional_t<BITS <= 32, float, double>, T>;\n"
                        "\n"
                        "template<typename Me, typename T>\n"
                        "concept NotMe = !std::is_same_v<Me, T>;\n"
                        "\n"
                        "template<typename T>\n"
                        "concept Compressed = std::is_same_v<typename T::CompressedTag, void>;\n"
                        "\n"
                        "template<typename Me, typename T>\n"
                        "concept OtherCompressed = NotMe<Me, T> && Compressed<T>;\n"
                        "\n"
                        "template<typename _T, u32 BITS = sizeof(_T) * 8, u32 ALIGNMENT = std::max(1U, std::bit_ceil(BITS) / 8)> requires std::integral<_T> || std::floating_point<_T>\n"
                        "class alignas(ALIGNMENT) CompressedNum : public CompressedVal<Native<_T, BITS>, BITS, ALIGNMENT, std::is_integral_v<_T>> {\n"
                        "    using Base = CompressedVal<Native<_T, BITS>, BITS, ALIGNMENT, std::is_integral_v<_T>>;\n"
                        "    using T = Native<_T, BITS>;\n"
                        "\n"
                        "public:\n"
                        "    using CompressedTag = void;\n"
                        "    using Base::Base;\n"
                        "\n"
                        "    template<typename X>\n"
                        "    requires std::integral<X> || std::floating_point<X> || OtherCompressed<CompressedNum, X>\n"
                        ...
                        "    constexpr inline CompressedNum operator op(i32) { \\\n"
                        ...
                        "    template<typename X> requires std::integral<X> || std::floating_point<X> || OtherCompressed<CompressedNum, X>   \\\n"
                        "    constexpr inline CompressedNum &operator STRCAT(op, =) (const X &scalar) {                                      \\\n"
                        "        this->__set(this->__get() op scalar);                                                                       \\\n"
                        "        return *this;                                                                                               \\\n"
                        "    }                                                                                                               \\\n"
                        "                                                                                                                    \\\n"
                        "    template<typename X> requires std::integral<X> || std::floating_point<X> || OtherCompressed<CompressedNum, X>   \\\n"
                        "    constexpr inline X operator op (const X &scalar) const {                                                        \\\n"
                        "        return (X) *this op scalar;                                                                                 \\\n"
                        "    }                                                                                                               \\\n"
                        "                                                                                                                    \\\n"
                        "    template<typename X> requires std::integral<X> || std::floating_point<X>                                        \\\n"
                        "    constexpr inline friend X& operator STRCAT(op, =) (X &scalar, const CompressedNum &cf) {                        \\\n"
                        "        scalar STRCAT(op, =) cf.__get();                                                                            \\\n"
                        "        return scalar;                                                                                              \\\n"
                        "    }                                                                                                               \\\n"
                        "                                                                                                                    \\\n"
                        "    template<typename X> requires std::integral<X> || std::floating_point<X>                                        \\\n"
                        "    constexpr inline friend X operator op (const X &scalar, const CompressedNum &cf) {                              \\\n"
                        ...
                        "    constexpr inline CompressedNum &operator STRCAT(op, =) (const CompressedNum &scalar) {  \\\n"
