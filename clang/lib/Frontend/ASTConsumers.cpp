...
namespace {
  ...
  public:
    ...
    bool VisitFieldDecl(FieldDecl *decl) {
      auto *parentRecord = decl->getParent();
      if (!RewriterASTConsumer::isParticleRecordDecl(parentRecord)) return true;
      auto name = decl->getNameAsString();
      auto nameRange = SourceRange(decl->getLocation(), decl->getLocation().getLocWithOffset(name.length() - 1));
      R.ReplaceText(nameRange, name + " : 1");
      return true;
    }
  };
